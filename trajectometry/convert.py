...
try:
    ...
    curs.execute("CREATE TABLE Positions"
                 " (id INTEGER PRIMARY KEY AUTOINCREMENT, cle BIGINT, heure INTEGER,"
                 " endroit INTEGER, FOREIGN KEY(cle) REFERENCES Personnes(cle))")
...
for k in ancien:
    ...
    curs.execute("INSERT INTO Personnes (cle, secteur, age, redressement, occupation) VALUES"
                 " (?,?,?,?,?)", (k[0], k[1], k[2], k[3], k[4]))
    # Ce sont les noms de colonnes des heures
    heures = [int(x) for x in cols[5:]]
    for i, j in enumerate(k[5:]):
        curs.execute("INSERT INTO Positions (cle, heure, endroit) VALUES"
                     " (?,?,?)", (k[0], heures[i], j))
...
if 96*pers == pos:
    ...
    warnings.warn(f"{pers} personnes converties mais {pos} positions,"
                  f" au lieu de {96*pers} (delta: {abs(96*pers-pos)}).")
